...
# Storage & reset variables
detections = []
reset_wait = 5               # seconds of no detection to reset
last_detection_time = time.time()
stable_detection = None
stable_start_time = None
stable_time_required = 1.0   # medium stability (~1 sec)
ready_to_store = True
...
while True:
    ...
    # -------------------------------
    # Detection + stability + single-store-per-reset + Arduino + LCD
    # -------------------------------
    current_detection=None
    indicator_text=""
    if prediction and dominant_color:
        current_detection=(prediction,dominant_color)
        if stable_detection==current_detection:
            elapsed=time.time()-stable_start_time
            if elapsed>=stable_time_required:
                indicator_text=f"STABLE: Ready to store {current_detection[0]}:{current_detection[1]}"
                if ready_to_store:
                    # Store detection
                    detections.append(current_detection)
                    print(f"{current_detection[0]}:{current_detection[1]}")
                    with open(CSV_FILE,mode="a",newline="") as f:
                        writer = csv.writer(f)
                        writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"),
                                         current_detection[0], current_detection[1]])
                    ready_to_store=False

                    # --- Arduino Servo Control ---
                    if arduino:
                        if current_detection[0].lower()=="amazon":
                    ...
                    if current_detection[0].lower()=="amazon":
                        ...
                        if arduino:
                            lcd_message = f"Amazon: {amazon_count}\n"
                            arduino.write(lcd_message.encode())
        else:
            stable_detection=current_detection
            stable_start_time=time.time()
            indicator_text=f"DETECTED: {current_detection[0]}:{current_detection[1]}"
    else:
        stable_detection=None
        stable_start_time=None
    ...
    if indicator_text:
        cv2.putText(frame,indicator_text,(10,30),
